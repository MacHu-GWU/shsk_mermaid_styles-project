...
| Working Backwards Chain | Zero labels in an unbranching chain | Every arrow in the derivation is labeled `requires` | `RL` alone does not tell a reader the arrow means requires, and the pattern ships a second diagram over the same nodes whose arrows mean `then`. Without the label the pair reads as two diagrams contradicting each other |
