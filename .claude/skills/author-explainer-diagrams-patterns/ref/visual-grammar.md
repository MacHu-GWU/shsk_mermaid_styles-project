...
Most shipped patterns use no emoji at all, which is the intended resting state rather than an oversight, and 💡 is currently used by nothing. Reach for one only when a pattern has a role none of its shapes can carry, which so far has meant marking a deliverable or a possession.
...
Several of these bend per pattern, most often the color budget and the label limits, and section 10 says where. Check the pattern file's own rules before failing a diagram on any line above.
...
| :--- | :--- | :--- | :--- |
| Step Flow | Stadium means start, entry point | Stadium marks both terminals, told apart by position and color | A bounded run has two ends of the same kind. `dbl-circ` is unavailable because a process repeated next week reaches no goal, and a `rounded` end would make the chain trail off rather than close. Safe because the pattern is strictly linear, so the two stadiums can never be confused |
...
| Timeline | The grammar has no diagram title | `title` is required | The window a timeline covers is its main argument and is otherwise invisible, since a reader cannot see that the axis starts in 2025 rather than at the company's founding |
...
| IO Pipeline | One self-contained diagram per pattern | The unit is a set: one overview plus one block per step, none publishable alone | A fan-in drawn on one canvas becomes wires crossing wires, so splitting is the whole idea, and the ID prefixes below are what re-join the pieces |
| IO Pipeline | 4 color classes | No color at all | Any artifact worth emphasizing is consumed by several blocks, so its highlight would repeat across the unit and become indistinguishable from color marking provenance, which the pattern bans outright. Provenance is the ID prefix instead |
...
| Proportion Pie | The grammar has no diagram title | `title` is required | Every slice is a share of one denominator, and the title is the only place the diagram can say what that denominator is. Same argument that made Timeline's window a required title |
...
| Cycle | `lean-r` is an input or prerequisite | Lean right holds the starter, a one-time event that set the ring moving, as `Slipped Deadline` | It is still the shape for what enters from outside, which is the part that carries, but a shock is an occurrence rather than a possession. Bounded to one, always grey, always on a dotted edge, so it reads as dispensable rather than as a standing requirement |
...
Green and red are crowded, and both already carry a second meaning held in common by several patterns: green marks "the one this is about" in the map patterns, and red marks an avoided outcome rather than a hazard, capped at one node. A new pattern joining either family copies it exactly and says so, because two patterns binding one color to two meanings is the drift a closed palette exists to prevent.

Watch what a row would make false elsewhere. A deviation that widens a shape's meaning belongs in section 2 as a wider definition rather than here as an exception, since leaving it here lets a later row call that shape reserved when it no longer is.
