...
| Quadrant | No pattern ships an `%%{init}%%` directive, and Timeline's row above calls them unsafe | Every quadrant diagram opens with `%%{init: {"quadrantChart": {"xAxisPosition": "bottom"}}}%%`, and the four quadrant labels are ALL CAPS | With points present, Mermaid stacks the title, the x axis poles, and the quadrant labels in three same-styled rows across the top, and no plain syntax separates axis furniture from verdicts. This is a layout key, not the theme variables Timeline rejected: a renderer that ignores it falls back to the top position, a cosmetic regression rather than a silently wrong color. The caps are the second layer, keeping verdicts legible as stamps even in that fallback |
